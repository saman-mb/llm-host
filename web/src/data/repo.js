export default {
  repo: {
    name: "llm-host",
    description: "Local LLM serving on a Framework Desktop (AMD Strix Halo, 125 GB unified RAM). Runs llama.cpp's llama-server inside a Vulkan toolbox container via llama-swap router, exposed as an OpenAI-compatible API on 0.0.0.0:8080 for agent clients on the LAN.",
    commits: 7,
    files: 21,
    language: "Bash",
    license: "Proprietary",
  ...
  layers: [
    ...
    {
      num: 7,
      title: "Process Supervision — llama-swap + systemd",
      summary: "llama-swap router (systemd user service) with Restart=always, Linger=on for boot without login",
      details: ["llama-swap manages model lifecycle — load on demand, swap between models", "Restart=always — recovers from crashes after 10s", "Linger=on via loginctl enable-linger", "config regenerated by scripts/gen-swap-config.sh from config.sh"],
      callout: "Service starts at boot without anyone logged in. llama-swap handles model switching without manual restarts."
    },
    {
      num: 8,
  ...
  config: {
    ...
    vars: [
      ...
      { name: "MODELS", value: "qwen3-next-80b|..., gemma-4-26b|..., qwen3.6-35b-a3b|...", desc: "Model registry: key|path pairs; first entry is the default" },
      { name: "HOST", value: "0.0.0.0", desc: "0.0.0.0 for LAN, 127.0.0.1 for local-only" },
      { name: "PORT", value: "8080", desc: "TCP port" },
      { name: "CONTEXT", value: "262144", desc: "Total token budget across all slots (per-slot = CONTEXT / N_PARALLEL)" },
      { name: "N_PARALLEL", value: "1", desc: "Concurrent request slots (each gets CONTEXT/N_PARALLEL tokens)" },
      { name: "EXTRA_FLAGS", value: "-ngl 999 -fa 1 --no-mmap --jinja --reasoning auto --reasoning-format auto --reasoning-budget 2048 --timeout 3600 --cache-ram 0 --no-cache-idle-slots --ctx-checkpoints 0 --checkpoint-every-n-tokens -1", desc: "Pass-through to llama-server (via llama-swap)" }
    ]
  },
